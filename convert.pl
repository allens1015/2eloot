  ...
  foreach my $tbody ($source->findnodes("//tbody")) {
    ...
    foreach my $tr ($source->findnodes("//tr")) {
      ...
      foreach my $td ($tr->findnodes("./td")) {
        ...
        if($i eq 6) {
          my @subcategories = $td->findnodes("./span/u/a");
          my @subcategories_str_container;
          foreach my $subcategory (@subcategories) {
            push(@subcategories_str_container,$subcategory->textContent);
          }
          my $subcategories_str = join(", ",@subcategories_str_container);
          $row{subcategory} = $subcategories_str;
        }
        if($i eq 7) {
