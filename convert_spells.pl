...
foreach my $file (glob("data/html_table/spells.html")) {
  ...
  foreach my $tbody ($source->findnodes("//tbody")) {
    ...
    foreach my $tr ($source->findnodes("//tr")) {
      ...
      foreach my $td ($tr->findnodes("./td")) {
        ...
        if($i eq 3) {
          my @traditions = $td->findnodes("./span/u/a");
          my @traditions_str_container;
          foreach my $tradition (@traditions) {
            push(@traditions_str_container,$tradition->textContent);
          }
          my $traditions_str = join(", ",@traditions_str_container);
          $row{traditions} = $traditions_str;
        }
        if($i eq 4) {
        ...
        if($i eq 8) {
          my $hash_data = 0;
          if($td->textContent) { $hash_data = $td->textContent; }
          $row{level} = $hash_data;
        }
